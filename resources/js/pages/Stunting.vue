...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'Stunting', href: '/stunting' },
];

const search = ref('');
const searchCategory = ref('nama'); // default pencarian

// Kosongkan data stunting
...
const editData = (id: number) => {
  alert(`Edit data stunting dengan ID: ${id}`);
};

const hapusData = (id: number) => {
  const confirmDelete = confirm(`Apakah Anda yakin ingin menghapus data stunting dengan ID: ${id}?`);
  if (confirmDelete) {
        ...
        <table class="min-w-full table-auto border border-gray-300 rounded-lg">
          ...
          <tbody>
            <tr v-if="filteredStunting.length === 0" class="text-center text-gray-500">
              <td colspan="5" class="px-4 py-4">Tidak ada data stunting.</td>
            </tr>
            <tr
            ...
            >
              ...
              <td class="px-4 py-2">{{ data.statusStunting }}</td>
