...
<template>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <table class="min-w-full">
          <thead class="bg-orange-500 text-white">
            <tr>
              ...
              <th v-if="user.role !== 'admin'" class="px-4 py-2 text-center text-sm">Aksi</th>
            </tr>
          </thead>
          <tbody>
    ...
    <button
      @click="deletePenduduk(p.nik)"
      class="p-1 bg-red-500 text-white rounded hover:bg-red-600 ml-2"
      title="Hapus"
    >
  ...
  <transition name="fade">
  <div v-if="showModal" class="fixed inset-0 z-50 flex items-start justify-center bg-gray-200/50">
  <div class="bg-white border border-orange-500 rounded-xl shadow-lg p-4 w-full max-w-sm text-center mt-10">
    ...
    <h2 class="text-xl font-bold text-orange-600 mb-4">Konfirmasi Hapus</h2>
    <p class="text-black text-sm mb-1">Apakah Anda yakin ingin menghapus data ini?</p>
    <div class="flex justify-center gap-3">
      <button @click="showModal = false" class="bg-gray-500 text-white text-sm px-4 py-1 rounded-sm hover:bg-gray-600">Batal</button>
      <button @click="deleteConfirmed" class="bg-red-600 text-white text-sm px-4 py-1 rounded-sm hover:bg-red-700">Hapus</button>
    </div>
  </div>
</div>
  </transition>
