...
const submitPendudukBaru = () => {
  formIbu.post('/penduduk', {
    ...
    preserveState: true,
    onSuccess: () => {
      form.penduduk_ibu_nik = formIbu.nik; // Simpan NIK Ibu
...
const goToNextStep = () => {
  ...
  // Hanya melanjutkan ke langkah berikutnya jika nama Ibu ditemukan
  if (step.value === 1 && namaIbu.value) {
    // Tidak mengosongkan NIK Ibu di sini
  }
...
const submitForm = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Mengirim data setelah mendapatkan lokasi
          form.post('/baduta', {
            onSuccess: () => {
  ...
  // Mengirim data jika semua sudah siap
  form.post('/baduta', {
    onSuccess: () => {
      window.location.href = '/penduduk';
    },
    ...
    <div class="p-6">
      <div class="bg-white shadow rounded p-4 relative">
        ...
        <form @submit.prevent="submitForm">
          <div class="mb-6 max-w-md" v-if="step === 1">
