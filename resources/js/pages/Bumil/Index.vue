...
const filteredBumils = computed(() => {
  ...
  return bumils.value.filter((bumil) => {
    return bumil.nama.toLowerCase().includes(keyword) || 
           bumil.nik.toLowerCase().includes(keyword);
  });
});

const paginatedBumils = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  return filteredBumils.value.slice(start, start + itemsPerPage.value);
});

const totalPages = computed(() => Math.ceil(filteredBumils.value.length / itemsPerPage.value));
    ...
    <div class="p-6">
      ...
      <div class="overflow-x-auto border border-orange-500 rounded-lg shadow">
        <table class="min-w-full">
          ...
          <tbody>
            <tr v-if="paginatedBumils.length === 0">
              <td colspan="8" class="text-center py-4 text-gray-500 text-sm">Tidak ada data bumil.</td>
            </tr>
            <tr
              v-for="(bumil, index) in paginatedBumils"
              :key="bumil.nik"
              class="hover:bg-orange-50 border-t border-orange-500"
            >
              <td class="px-4 py-2 text-sm">{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td>
              <td class="px-4 py-2 text-sm">{{ bumil.nik }}</td>
              <td class="px-4 py-2 text-sm">{{ bumil.nama }}</td>
              ...
              <td v-if="user.role !== 'TPK'" class="px-4 py-2 text-sm">{{ bumil.niktpk }}</td>
