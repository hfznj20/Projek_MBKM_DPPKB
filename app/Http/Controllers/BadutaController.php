    ...
    public function locations()
{
    ...
    // Ambil lokasi BUMIL dengan join ke tabel penduduk
    $bumil = DB::table('bumil')
        ->join('penduduk', 'bumil.penduduk_nik', '=', 'penduduk.nik')
        ->where('penduduk.kategori', 'BUMIL')
        ->select('bumil.id', 'penduduk.nama', 'bumil.latitude', 'bumil.longitude', 'bumil.penduduk_nik')
        ->get()
        ->map(function ($item) {
            return [
                'id' => $item->id,
    ...
    // Gabungkan hasil BADUTA dan BUMIL
    $locations = $baduta->merge($bumil);
